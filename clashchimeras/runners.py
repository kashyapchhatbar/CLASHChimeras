...
class Bowtie:
  ...
  def __init__(self, args=None):
    ...
    if os.path.exists(self.uniqueInput) and os.path.exists(
      ...
      if s == _s:
        logger.info('{} is present and verified'.format(self.uniqueInput))
      else:
        logger.warn('{} is present but checksum verification failed'.format(
          self.uniqueInput))
        logger.warn('{} will be generated again'.format(self.uniqueInput))
    else:
  ...
  def run(self, output=None, filtered=None):
    ...
    if os.path.exists(self.outputSam) and os.path.exists(
      ...
      if s == _s:
        logger.info('{} is present and verified'.format(self.outputSam))
    ...
    if self.runAligner:
      ...
      if os.path.exists(self.outputSam):
        logger.info('Bowtie2 execution finished and {} generated'.format(
          self.outputSam))
        ...
        logger.error('Please check your bowtie2 stderr and report a bug....')
